...
- `backend/` — Express + TS + Prisma (Postgres) + IPFS receipts. Frontend works without it except attaching proof. Spec: `docs/04-BACKEND.md`.
...
- `backend/.env` holds the Postgres URL and Pinata JWT. Never echo them into docs, examples, or commits. Never run `pnpm seed` against the real DB.
